...
*求解平方积分图
*/
void GetGrayIntegralImage(int *Src, double *Integral, int Width, int Height, int Stride)
{
	double *ColSum = (double *)calloc(Width, sizeof(double));        //    用的calloc函数，自动内存清0
	memset(Integral, 0, (Width + 1) * sizeof(double));
	for (int Y = 0; Y < Height; Y++)
	{
		int *LinePS = Src + Y * Stride;
		double *LinePL = Integral + Y * (Width + 1) + 1;
		double *LinePD = Integral + (Y + 1) * (Width + 1) + 1;
		LinePD[-1] = 0;
		for (int X = 0; X < Width; X++)
		{
			ColSum[X] += LinePS[X];
...
//求解平方积分图
void GetGraySqImage(unsigned char *Src, double *Integral, int Width, int Height, int Stride)
{
