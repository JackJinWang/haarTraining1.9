...
//获取积分图和归一化因子
static
MyMat* myGetAuxImages(const MyMat *src, MyMat *sumpic, MyMat *sqsumpic, float &normfactor, MySize winsize)
{
	GetGrayIntegralImage(src->data.ptr, sumpic->data.i, winsize.width, winsize.height, src->step);
	GetGraySqImage(src->data.ptr, sqsumpic->data.db, winsize.width, winsize.height, src->step);
	...
	double area = 0.0;
	normrect = { 1, 1, src->cols - 2, src->rows - 2 };
	CV_SUM_OFFSETS(p0, p1, p2, p3, normrect, src->cols + 1)
		area = normrect.width * normrect.height;
	valsum = ((sum_type*)(sumpic->data.ptr))[p0] - ((sum_type*)(sumpic->data.ptr))[p1]
		- ((sum_type*)(sumpic->data.ptr))[p2] + ((sum_type*)(sumpic->data.ptr))[p3];
	valsqsum = ((sqsum_type*)(sqsumpic->data.ptr))[p0]
		- ((sqsum_type*)(sqsumpic->data.ptr))[p1]
		- ((sqsum_type*)(sqsumpic->data.ptr))[p2]
		+ ((sqsum_type*)(sqsumpic->data.ptr))[p3];
...
static void getPicture(CvHaarTrainingData* training_data, int *number_all, int number, int flag, MySize mysize)
{
	...
	{
		MyMat *tempMat = createMyMat(mysize.height, mysize.width, ONE_CHANNEL, UCHAR_TYPE);                                  //注意最后要释放	
		MyMat *tempSum = createMyMat(mysize.height + 1, mysize.width + 1, ONE_CHANNEL, INT_TYPE);//注意最后要释放																	 //  MyMat *tempTitle = createMyMat(mysize.height + 1, mysize.width + 1, ONE_CHANNEL, INT_TYPE);//注意最后要释放
		...
		for (int i = 0;i < number;i++)
		{
			...
			if (tempMat != nullptr)
			{
				...
				//		myIntegral(tempMat, tempSum, tempTitle, tempSqsum);
				int *address = training_data->sum.data.i;
				tempSum = myGetAuxImages(tempMat, tempSum, tempSqsum, normfactor, mysize);
				training_data->normfactor.data.fl[trainingdata_number] = normfactor;
				cout << trainingdata_number << ":" << training_data->normfactor.data.fl[trainingdata_number] << endl;
				//积分图复制到training_data中
	...
	int numprecalculated, int fileOrMem, const char* filedirname)
{
	...
	case SAVE_FEATURE_FILE:
	{
		...
		float *valarry = new float[num_samples];
		...
		for (int i = 0; i < haarFeatures->count; i++)
		{
			for (int j = 0; j < num_samples; j++)
			{
				...
				normfactor = data->normfactor.data.fl[j];
				valarry[j] = (normfactor == 0.0F) ? 0.0F : (val / normfactor);
				idxarry[j] = j;
			}
			quickSort(valarry, idxarry, 0, num_samples - 1);
			for (int j = 0;j < num_samples;j++)
			{
				file.write((char*)&valarry[j], sizeof(float));
...
void saveXML(CvIntHaarFeatures* haarFeatures, int stage, vector<MyStumpClassifier> strongClassifier, const char* dirname, float stage_thresold)
{
	...
	for (int i = 0;i < strongClassifier.size();i++)
	{
		...
		*/
		...
		float b = weakClassifier.error / (1 - weakClassifier.error);
		float temp = logf(1 / b);
		sunElement2->SetText(temp);
	...
	*/
	...
	for (int i = 0;i < picturePosNum;i++)
	{
		...
		for (int j = 0;j < length;j++)
		{
			...
			normfactor = haarTrainingData->normfactor.data.fl[i];
...
*/
static
int predictSignal(CvIntHaarFeatures* haarFeatures, MyMat* pic, MySize size, MyCascadeClassifier classifier,float normfactor)
{
...
static int replaceNegPictrueStrengthen(CvIntHaarFeatures* haar_features, MyCascadeClassifier classifier, CvHaarTrainigData* training_data,int postion, int &replace_postion, MySize mysize)
{
	...
	if (tempMat != nullptr)
	{
		...
		/*
		GetGrayIntegralImage(tempMat->data.ptr, tempSum->data.i, mysize.width, mysize.height, tempMat->step);
		GetGraySqImage(tempMat->data.ptr, tempSqsum->data.db, mysize.width, mysize.height, tempMat->step);
		MyRect normrect;
		int p0, p1, p2, p3;
		sum_type   valsum = 0;
		sqsum_type valsqsum = 0;
		double area = 0.0;
		normrect = { 1, 1, tempMat->cols - 2, tempMat->rows - 2 };
		CV_SUM_OFFSETS(p0, p1, p2, p3, normrect, tempMat->cols + 1)

			area = normrect.width * normrect.height;
		valsum = ((sum_type*)(tempSum->data.ptr))[p0] - ((sum_type*)(tempSum->data.ptr))[p1]
			- ((sum_type*)(tempSum->data.ptr))[p2] + ((sum_type*)(tempSum->data.ptr))[p3];
		valsqsum = ((sqsum_type*)(tempSqsum->data.ptr))[p0]
			- ((sqsum_type*)(tempSqsum->data.ptr))[p1]
			- ((sqsum_type*)(tempSqsum->data.ptr))[p2]
			+ ((sqsum_type*)(tempSqsum->data.ptr))[p3];
		...
		*/
		...
		return NEGISOVER;
	int result = 1; //需要循环
    if (predictSignal(haar_features, tempSum, classifier.size, classifier,normfactor) == 1 ) //fp 样本需要加强
	{
		address = postion * (mysize.width + 1)*(mysize.height + 1) + address;
		memcpy(address, tempSum->data.i, sizeof(int)*(mysize.width + 1)*(mysize.height + 1));
		result = 0;
	...
	//	cout << "n" << replace_postion << endl;
		...
		//负样本错误检测 fp
		if (predictSignal(haar_features, tempSum, classifier.size, classifier,normfactor) == 1)
		{
...
static int replacePosPictrueStrengthen(CvIntHaarFeatures* haar_features, MyCascadeClassifier classifier, CvHaarTrainigData* training_data, int postion, int &replace_postion, MySize mysize)
{
	...
	if (tempMat != nullptr)
	{
		...
		*/
		...
		return POSISOVER;
	int result = 1; //需要循环
	if (predictSignal(haar_features, tempSum, classifier.size, classifier, normfactor) == 1) //tp 样本需要加强
	{
		address = postion * (mysize.width + 1)*(mysize.height + 1) + address;
		memcpy(address, tempSum->data.i, sizeof(int)*(mysize.width + 1)*(mysize.height + 1));
		result = 0;
	...
	const char* dirname, float minhitrate, float maxfalsealarms, MySize mysize, int numprecalculated)
{
	...
	float *vector_feat = new float[sampleNumber];              //特征向量，记住要释放
		...
		while (current_falsealarms > maxfalsealarms && (!maxweaksplits || (featurenumber < maxweaksplits)))
		{
			...
			else
			{
				...
				for (int ll = 0;ll < sampleNumber;ll++)
				{
					...
					float normfactor = haarTrainingData->normfactor.data.fl[ll];
					val = (normfactor == 0.0F) ? 0.0F : (val / normfactor);
			...
			int nIdxRead;
			while (nRead = istream.read((char*)&nNum, sizeof(float)).gcount())
			{
				nIdxRead = idxstream.read((char*)&nidx, sizeof(int)).gcount();  //读入标号
				if (sizeof(float) != nRead)
					break;
